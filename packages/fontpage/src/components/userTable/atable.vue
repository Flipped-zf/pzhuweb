<template>
	<div>
		<el-table v-loadmore="myLoad" size="large" :data="data.ac" max-height="530" style="width: 100%">
			<template #empty>
			...
			<el-table-column min-width="100" label="技术标签" prop="AchievementType.name">
				<template #default="scope">
					<el-tag class="ml-2" type="success">{{ scope.row.AchievementType.name }}</el-tag>
				</template>
			</el-table-column>
...
function ConfirmDel() {
	user.deleteAchievement(currentArticl.id).then((res) => {
		if (res.data.success) {
