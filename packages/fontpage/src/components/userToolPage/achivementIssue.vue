<template>
	<div class="resourceIssue-container" v-loading="issueData.loading">
		<div class="header">成果分享 <small>(链接或附件二选一)</small></div>
		<div class="body">
			<div class="bodyLeft">
				<el-form ref="rformRef" :model="issueData" label-width="120px" size="large" :rules="rules">
					<el-form-item prop="link">
						<el-input v-model="issueData.achievementlink" :disabled="!!issueData.attachment" placeholder="分享成果链接：http://www.pzhuweb.cn" />
					</el-form-item>
					<el-form-item prop="title">
						<el-input v-model="issueData.title" placeholder="请输入成果标签" />
					</el-form-item>
					<el-form-item prop="abstract">
						<el-input v-model="issueData.abstract" :autosize="{ minRows: 2, maxRows: 4 }" type="textarea" placeholder="成果描述(120字以内)" />
					</el-form-item>
			...
			<div class="bodyRight">
				...
				<div class="fujian">
					<el-upload v-if="!myFlag2" ref="uploadp" :auto-upload="false" accept=".pdf" :limit="1" :on-change="handlechange2">
						<el-tooltip class="box-item" effect="dark" content="请上传pdf格式的文件" placement="bottom">
							<el-button :disabled="!!issueData.achievementlink" type="primary" :icon="Upload">上传成果附件</el-button>
						</el-tooltip>
					</el-upload>
...
import aApi from '../../api/achievementIssue';
...
function myvalidate() {
	const flag = !!issueData.achievementlink || !!issueData.attachment;
	return flag && !!issueData.posterlink;
}
async function handelIssue() {
	...
	await aApi.uploadAchievement(data).then((res) => {
...
aApi.getAchievementIssue(issueData.id).then((res) => {
	console.log(res);
	if (res.success) {
		...
		issueData.abstract = res.data.achievement[0].introduction;
		issueData.achievementType = res.data.achievementType;
		issueData.posterlink = res.data.achievement[0].posterlink;
		issueData.attachment = res.data.achievement[0].attachment;
		issueData.status = 2;
		issueData.date = res.data.achievement[0].created_at ? new Date(res.data.achievement[0].created_at) : new Date();
	}
});
